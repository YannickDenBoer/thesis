...
class Processor:
    ...
    def create_answer_embeddings(self, frame, question_list):        
        ...
        for label, question, pos_answer, neg_answer in question_list:
            try:
                gen_text = self.vl_model.generate(question, frame)
                pred_embed = self.vl_model.forward_pass(f"{gen_text}")
    ...
    def true_answers_to_embeddings(self):
        ...
        for label, question, pos_answer, neg_answer in question_list:
            true_embed = self.sentence_model.forward_pass(f"{pos_answer}")
            #true_embed = self.sentence_model.forward_pass(f"{question} [SEP] {pos_answer}")
            true_answer_embeddings.append(true_embed)
            del true_embed
            torch.cuda.empty_cache()
        true_answer_embeddings = torch.stack(true_answer_embeddings)
        return true_answer_embeddings # shape (num_questions, embedding_dim)

    def embeddings_to_cos_scores(self, answer_embeddings, true_answer_embeddings):
        true_answer_embeddings = true_answer_embeddings.unsqueeze(0)
        
        # Calculate cosine similarity for each sample and each question
        cos_scores = F.cosine_similarity(answer_embeddings, true_answer_embeddings, dim=2)
