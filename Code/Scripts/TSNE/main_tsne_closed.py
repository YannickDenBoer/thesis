...
if __name__ == "__main__":
    ...
    for i in range(len(CUE_LIST)):
        ...
        plot_tsne(X_class, y, groups=groups, label_type='cue', label_names=CUE_LIST, cue_index=i, path=f"{results_folder}tsne/{CUE_LIST[i]}/all/")
        # Create image for groups on closed-question all vids
        plot_tsne(X_class, y, groups=groups, label_type='group', label_names=CUE_LIST, cue_index=i, path=f"{results_folder}tsne/{CUE_LIST[i]}/all/")
        plot_tsne(X_class, y, groups=groups, label_type='time', label_names=CUE_LIST, cue_index=i, path=f"{results_folder}tsne/{CUE_LIST[i]}/all/")
        for group in unique_groups:
            ...
            plot_tsne(X_group_class, y_group_class, label_type='cue', label_names=CUE_LIST, cue_index=i, path= f"{results_folder}tsne/{CUE_LIST[i]}/{group}/")
            plot_tsne(X_group_class, y_group_class, label_type='time', label_names=CUE_LIST, cue_index=i, path= f"{results_folder}tsne/{CUE_LIST[i]}/{group}/")